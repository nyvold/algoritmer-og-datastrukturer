...
class Binaryheap:
    ...
    def insert(self, v: int):
        self.arr.append(v)#legger nytt element sist i heapen
    ...
    def removeMin(self) -> int:
        ...
        i = 0
        while self.rightOf(i) < len(self.arr)-2:#mens i har et høyrebarn, altså vi er ikke utfor arrayet og vi har garantert et venstrebarn
            j = self.leftOf(i) if self.arr[self.leftOf(i)] <= self.arr[self.rightOf(i)] else self.rightOf(i)
    ...
    def parentOf(self, i: int) -> int:
        return (i-1)//2
    
    def leftOf(self, i: int) -> int:
        return 2*i+1
    
    def rightOf(self, i: int) -> int:
        return 2*i+2
